...
def build_graph():
    ...
    images_HR = reconstruct_image(images, (featH), (2-featW), input_size, aspect_ratio_HR)
...
def model_test(sess, images, salmap, images_new, train_step, aspect_ratio, input_size):
    img_fnames = [os.path.join('test_image/', x) for x in os.listdir('test_image/') if x.endswith('.jpg')]
    base_fnames = [os.path.splitext(os.path.basename(x))[0] + '.jpg' for x in img_fnames]
    ...
    for i, (imgPath, imgName) in enumerate(zip(img_fnames, base_fnames)):
        ...
        #     imsave('./test_image_retargeting' + '/step' + str(TR[1]) + '_img_' + imgName, images_new_[0, ...])
        #     imsave('./test_image_retargeting' + '/step' + str(TR[1]) + '_sal_' + imgName, imresize(salmap_[0,:,:],
        #                                                                                            [shape[1], shape[2]], 'bicubic'))

        ## Zoom in
        target_ratio = ((shape[1], shape[2]*1.25), (shape[1], shape[2]*1.5), (shape[1], shape[2]*1.75), (shape[1], shape[2]*2))
...
def main(argv=None):
    ...
    with tf.Session() as sess:
        ...
        # for i in range(300):
        model_test(sess, images, salmap, images_LR, 1, aspect_ratio, input_size)

...
def spatial_transformer_network(input_fmap, theta, out_dims=None, **kwargs):
    ...
    # print(out_fmap)
    # out_fmap = tf.reshape(out_fmap, shape=[B, H, W, C])
...
def affine_grid_generator(height, width, theta):
    ...
    x = tf.range(-1.0, 1.0, 2/width)
    y = tf.range(-1.0, 1.0, 2/height)
    ...
    theta = tf.cast(theta, 'float32')
    batch_grids = tf.reshape(batch_grids, [num_batch, 2, tf.shape(y)[0], tf.shape(x)[0]])
...
if __name__ == '__main__':
    img = imread('/home/user/TF_project/image_retargeting_DL/RetargetMeAll/boat.png')
    img = np.stack((img, img, img, img, img, img))
    B, H, W, C = img.shape
    print(img.shape)

    images = tf.placeholder(tf.float32, shape=(B, H, W, C))
    aspect_ratio = tf.placeholder(tf.float32, shape=(4,))
    theta = tf.Variable(np.random.uniform(0, 1, (B, 6)), dtype=tf.float32)

    with tf.Session() as sess:
        ta_final_result = spatial_transformer_network(images, theta, out_dims=aspect_ratio)
        sess.run(tf.global_variables_initializer())

        # Runs the op.
        x = sess.run(ta_final_result, feed_dict={images: img, aspect_ratio: [B, H, W / 2, C]})

...
def main(argv=None):
    ...
    with tf.Session() as sess:
        ...
        for i in range(max_iter_step):
            ...
            feed_dict = {images: img, aspect_ratio: aspect_ratio_, input_size: input_size_}

            if i % 25 == 24:
                _, merged, generator_loss, Sh_, Sw_ = sess.run([opt_g, merged_all, CycleLoss, Sh, Sw], feed_dict=feed_dict)
                summary_writer.add_summary(merged, i)
                print(Sh_[0, :])
                print('featH mean: ', np.mean(Sh_, 1))
                print(Sw_[0, :])
                print('featW mean: ', np.mean(Sw_, 1))
            else:
